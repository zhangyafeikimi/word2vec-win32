...
void SortVocab() {
  ...
  train_words = 0;
  for (a = 1; a < size; a++) {
    // Words occurring less than min_count times will be discarded from the
    // vocab
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf(
        ...
        "higher frequency");
    printf(
        " in the training data will be randomly down-sampled; default is 0 "
        "(off), useful value is 1e-5\n");
    printf("\t-hs <int>\n");
    printf("\t\tUse Hierarchical Softmax; default is 1 (0 = not used)\n");
    printf("\t-negative <int>\n");
    printf(
        "\t\tNumber of negative examples; default is 0, common values are 5 - "
        "10 (0 = not used)\n");
    printf("\t-threads <int>\n");
    ...
    printf("\t-cbow <int>\n");
    printf(
        "\t\tUse the continuous bag of words model; default is 0 (skip-gram "
        "model)\n");
    printf("\nExamples:\n");
    printf(
        "./word2vec -train data.txt -output vec.txt -debug 2 -size 200 -window "
        "5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
